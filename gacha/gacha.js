...
async function prepareModelResources(gl)
{
  ...
  index.forEach(model_data => {
    modelRegistry.set(model_data.id, {
    ...
    const model = modelRegistry.get(model.id);
    ...
    const load_icon = (id) => {
      const path = "models/icons/"
    };

    const load_mesh = (mesh_name) => {
      const path = "models/"
    };
...
async function main() {
  ...
  const scene = {
    objects: [
      {
        model: modelRegistry.get(30008),
        buffers: null,
      },
      {
        model: modelRegistry.get(30035),
        buffers: null,
      },
      /*{
